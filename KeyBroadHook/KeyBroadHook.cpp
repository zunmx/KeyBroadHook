// KeyBroadHook.cpp : 定义应用程序的入口点。
//

#include "framework.h"
#include "KeyBroadHook.h"
#include "commons.h"
#include <shellapi.h>
	...
	_In_ int       nCmdShow)
{
	...
	LoadStringW(hInstance, IDC_KEYBROADHOOK, szWindowClass, MAX_LOADSTRING);
	...
	HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_KEYBROADHOOK));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_KEYBROADHOOK));
	wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
	wcex.hbrBackground = bgColor;
	wcex.lpszMenuName = MAKEINTRESOURCEW(IDC_KEYBROADHOOK);
...
void CreateTray(HWND hwnd) {
	...
	m_Notify.hIcon = LoadIcon(hInst, MAKEINTRESOURCE(IDI_KEYBROADHOOK));
	...
	m_Notify.uID = 1;
	lstrcpy(m_Notify.szTip, L"KeyBroadHook");
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		case IDM_EXIT:
			...
			break;
		case SETUP_HOOK:
			MessageBoxA(hWnd, KeyBroadHookOper(1), "提示", MB_OK | MB_ICONINFORMATION);
			break;
		case REMOVE_HOOK:
			MessageBoxA(hWnd, KeyBroadHookOper(0), "提示", MB_OK | MB_ICONINFORMATION);
			break;
		case REFRESH:
